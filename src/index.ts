...
function quickKeywordReply(text, env) {
  ...
  const contactMenu = [
    {
      ...
      text: [
        '📞 ช่องทางติดต่อหลัก',
        '• แม่บ้าน (พี่ก้อย) 080-649-0441',
        '• อยากคุยกับผู้จัดการหรือเจ้าของ กดปุ่มด้านล่างได้เลยค่ะ'
      ].join('\n'),
      quickReply: {
        items: [
          { type: 'action', action: { type: 'uri', label: 'โทรหาแม่บ้าน', uri: 'tel:0806490441' } },
          { type: 'action', action: { type: 'message', label: 'คุยกับผู้จัดการ', text: 'ติดต่อผู้จัดการ' } },
          { type: 'action', action: { type: 'message', label: 'คุยกับเจ้าของ', text: 'ติดต่อเจ้าของหอ' } },
        ]
      }
    }
  ];
  ...
  const maidContact = [
    {
      type: 'text',
      text: 'แม่บ้าน (พี่ก้อย) 080-649-0441\nโทรได้ทุกวัน 08:00-20:00 น.',
      quickReply: {
        items: [
          { type: 'action', action: { type: 'uri', label: 'โทรหาแม่บ้าน', uri: 'tel:0806490441' } },
          { type: 'action', action: { type: 'message', label: 'คุยกับผู้จัดการ', text: 'ติดต่อผู้จัดการ' } }
  ...
  const managerTemplate = [
    {
      type: 'text',
      text: 'ผู้จัดการ (พิม) พร้อมช่วยเหลือเรื่องสัญญา/การจองค่ะ\nพิมข้อความที่ต้องการไว้ได้เลย หรือกดปุ่มด้านล่างเพื่อให้เราติดต่อกลับ',
      quickReply: {
        items: [
          { type: 'action', action: { type: 'message', label: 'ฝากเรื่องถึงผู้จัดการ', text: 'ฝากเรื่องถึงผู้จัดการ' } },
          { type: 'action', action: { type: 'uri', label: 'โทรหาแม่บ้าน', uri: 'tel:0806490441' } },
          { type: 'action', action: { type: 'message', label: 'คุยกับเจ้าของ', text: 'ติดต่อเจ้าของหอ' } },
        ]
      }
  ...
  const ownerTemplate = [
    {
      type: 'text',
      text: 'เจ้าของหอพร้อมรับฟังข้อเสนอแนะและเรื่องพิเศษค่ะ\nฝากข้อความไว้ได้เลย ระบบจะแจ้งให้ทราบทันที',
      quickReply: {
        items: [
          { type: 'action', action: { type: 'message', label: 'ฝากเรื่องถึงเจ้าของ', text: 'ฝากเรื่องถึงเจ้าของหอ' } },
          { type: 'action', action: { type: 'uri', label: 'โทรหาแม่บ้าน', uri: 'tel:0806490441' } },
          { type: 'action', action: { type: 'message', label: 'คุยกับผู้จัดการ', text: 'ติดต่อผู้จัดการ' } },
        ]
      }
  ...
  if (['ติดต่อผู้จัดการ', 'ผู้จัดการ', 'คุยกับผู้จัดการ', 'ฝากเรื่องถึงผู้จัดการ'].includes(lower)) {
    return managerTemplate;
  }

  if (['ติดต่อเจ้าของหอ', 'เจ้าของหอ', 'คุยกับเจ้าของ', 'ฝากเรื่องถึงเจ้าของหอ'].includes(lower)) {
